...
public class MessageDAO extends DAO<Message> {
	...
	public Message addMessage(int userId, String text, boolean isPublic) throws SQLException {
		...
		String sql = "INSERT INTO messages (user_id, text, is_public, "
				   + "created_at, updated_at) "
				   + "VALUES(?, ?, ?, ?, ?)";

		PreparedStatement statement = this.db.prepareStatement(sql);
		statement.setInt(1, userId);
		statement.setString(2, StringEscapeUtils.escapeHtml4(text));
	...
	private List<Pair> createPairs(List<Message> messages) throws SQLException {
	...
	public List<Pair> findVisible(int userId) throws SQLException {
		List<Message> list = new ArrayList<Message>();

		String sql = "SELECT * FROM messages WHERE user_id = ? OR is_public = ? "
	...
	public Message updateMessage(int id, String text, boolean isPublic) throws SQLException {
		...
		String sql = "UPDATE messages SET text = ?, is_public = ?, updated_at = ? "
				   + "WHERE id = ?";

		PreparedStatement statement = this.db.prepareStatement(sql);
		statement.setString(1, StringEscapeUtils.escapeHtml4(text));
	...
	public void removeImage(int id) throws SQLException {
		...
		String sql = "UPDATE messages SET image_file_name = ?, image = ?, updated_at = ? WHERE id = ?";
		PreparedStatement statement = this.db.prepareStatement(sql);
